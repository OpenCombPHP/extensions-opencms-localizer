...
class OpencmsLocalizer extends Extension 
{
	...
	public function active()
	{
		...
		$this->mapDbTable( $aLocale );
		$this->mountSetting( $aLocale );
	}
	...
	private function mapDbTable( Locale $aLocale ){
		$sPrefix = DB::singleton()->tableNamePrefix();
	...
	private function mountSetting( Locale $aLocale ){
		$aExtOpencms = Extension::flyweight('opencms');
		$aExtOpencmsLocalizer = Extension::flyweight('opencms-localizer');
		
		$aSettingOpencms = $aExtOpencms->setting();
		$aSettingOpencmsLocalizer = $aExtOpencmsLocalizer->setting();
		
		$arrExportKeys = array('menu','index');
		
		foreach($arrExportKeys as $sExportKey){
			$aSettingOpencms->mount(
				$aSettingOpencmsLocalizer->separate(
					$aLocale->localeName().'/'.$sExportKey
				),
				'/'.$sExportKey
			);
		}
